...
## Description
This is official implementation of [PoLiCER](https://openreview.net/forum?id=ITeuGb2bYg&referrer=%5BAuthor%20Console%5D(%2Fgroup%3Fid%3DICLR.cc%2F2026%2FConference%2FAuthors%23your-submissions)) for vector-based state inputs. Our implementation is based on the official codebase of [B-Pref](https://github.com/rll-research/BPref) and [SURF](https://github.com/alinlab/SURF). We also benefit from [RUNE](https://github.com/rll-research/rune), [MRN](https://github.com/RyanLiu112/MRN), and [QPA](https://github.com/huxiao09/QPA). We appreciate their insightful works.
...
## Human-in-the-Loop Experiment Results (DMC Walker)
### DMControl Walker : PEBBLE(Left) and PoLiCER(Right)
<p align="center">
  <img src="./samples/pebble_vector_walker_human-clip.gif" align="center" width="32%">
  <img src="./samples/policer_vector_walker_human_clip.gif" align="center" width="32%">
</p>
...
docker run -it -d --shm-size=64g --gpus=all -v /your/drive/location:/mnt/hdd/workspace -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=unix$DISPLAY --env QT_X11_NO_MITSHM=1 --name PoLiCER_vector pytorch/pytorch:1.11.0-cuda11.3-cudnn8-runtime
apt-get update
...
## How to run
### Synthetic annotator experiment with vector-based observation (Walker Walk)
```bash
bash scripts/synthetic.sh
```

### Human-in-the-Loop experiment with vector-based observation
```bash
bash scripts/human.sh
```

If you wanna conduct HiL experiment with Meta-world environment, executing following command before will be helpful for addressing Missing GL version issue.
